# 🖥️ AI Chatbot with Browser Control
...
- Python 3.10+
- An OpenAI API key (get one free at [platform.openai.com](https://platform.openai.com/api-keys))
...
└── tests/
    ├── conftest.py             # Test config (mocks streamlit & openai)
    ├── test_error_handling.py  # Error classification tests
    └── test_browser_agent.py   # Tool dispatch and validation tests
```
...
23 tests covering error handling, tool dispatch, input validation, and URL normalization.
